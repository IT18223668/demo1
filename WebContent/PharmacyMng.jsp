...
<div class="super_container">
	...
	<%
		...
		hs.setAttribute("AllStock", stM.getAllItem());
...
<h2>Stock Update</h2>
<a href="stockRegistration.jsp">+ add new stock </a>
<table>
	<tr>
		<td width=100px>ItemNumber</td>
		<td width=100px>Item Name</td>	
		<td width=100px>Brand Name</td>
		<td width=100px>Supplier ID</td>
		<td width=100px>Supplier Name</td>
		<td width=100px>Price</td>
		<td width=100px>Quantity</td>
		<td width=100px>Order Level</td>
		<td width=100px>Per tablet</td>
		<td width=100px>per card</td>
		
	</tr>
	<%
		ArrayList<stock> stock1=(ArrayList<stock>) session.getAttribute("AllStock");
		for(stock st:stock1){
	%>
	
	<tr>
		<td
			 onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
		<%
			out.print(st.getItemNumber());
		%>
		</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getItemName());
			%>
		</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getBrandName());
			%>	
			</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getSupplierID());
			%>	
		</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getSupplierName());
			%>	
		</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getPrice());
			%>	
		</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getQuantity());
			%>	
		</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getSetReOrderLevel());
			%>	
		</td>
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getPerTablet());
			%>	
		</td>
		...
		<td
			onclick="location.href='PharmacyMng.jsp?id=<%=st.getItemNumber()%>'">
			<%
				out.print(st.getPerCard());
			%>	
		</td>
				<td> 
				<a href="StockEdits.jsp?u=<%=st.getItemNumber()%>">Edit</a>
			    </td>
			    <td>
			    	<form action="delete" method="post">
			    		<input type="hidden" value=<%=st.getItemNumber() %> name="StockID">
