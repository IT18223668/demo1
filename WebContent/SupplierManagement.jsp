				...
				<div class="row">
					<div class="col">
						<div class="home_content">
							<div class="home_title">Suppliers</div>
						</div>
					</div>
				</div>
			</div>
...
<!-- OPD Homepage -->
	...
	<%
		...
		supplier_service pat=new supplier_service_impl();
		HttpSession hs=request.getSession();
		hs.setAttribute("allsuppliers", pat.getSupplier());
	%>
	
	

	<br><a href="SupplierRegistration.jsp">+ Add new suppliers</a>
...
<h2>Suppliers</h2>
	...
	<tr>
		...
		<td width=100px>Join Date</td>
		<td width=100px>Items</td>
	</tr>
	<%
		ArrayList<suppliers> supplier=(ArrayList<suppliers>) session.getAttribute("allsuppliers");
		for(suppliers sp:supplier){
	%>
	
	<tr>
		<td
			 onclick="location.href='SupplierManagment.jsp?id=<%=sp.getSupplierID()%>'">
		<%
			out.print(sp.getSupplierID());
		%>
		</td>
			<td
			onclick="location.href='SupplierManagment.jsp?id=<%=sp.getSupplierID()%>'">
		...
		<td
			onclick="location.href='SupplierManagment.jsp?id=<%=sp.getSupplierID()%>'">
			<%
				out.print(sp.getAddress());
			%>	
			</td>
		<td
			onclick="location.href='SupplierManagment.jsp?id=<%=sp.getSupplierID()%>'">
			<%
				out.print(sp.getContactNumber());
			%>	
		</td>
		<td
			onclick="location.href='SupplierManagment.jsp?id=<%=sp.getSupplierID()%>'">
			<%
				out.print(sp.getEmail());
			%>	
			</td>
			<td
			onclick="location.href='SupplierManagment.jsp?id=<%=sp.getSupplierID()%>'">
			<%
				out.print(sp.getJoin_date());
			%>	
			</td>
			<td
			onclick="location.href='SupplierManagment.jsp?id=<%=sp.getSupplierID()%>'">
			<%
				out.print(sp.getItem());
			%>	
			</td>
			
			<td> 
				<a href="SupplierEdits.jsp?u=<%=sp.getSupplierID()%>">Edit</a>
			    </td>	
			    <td>
			    	<form action="delete" method="post">
			    		<input type="hidden" value=<%=sp.getSupplierID()  %> name="SupID">
