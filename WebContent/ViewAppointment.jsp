...
<!-- OPD Homepage -->
	...
	<%
		...
		hs.setAttribute("allpatients", pat.getAllPatients());
	%>
	
	

	<br><a href="PatientRegistration.jsp">+ Add new patient</a>
...
<h2>Patients</h2>
	...
	<table>
		<tr><td><input type="text" name="pid" placeholder="search old patient" required></td></tr>
	...
	<tr>
		...
		<td width=100px>Diagnose</td>
		<td width=100px>Drugs</td>
	</tr>
	<%
		ArrayList<patients> patient=(ArrayList<patients>) session.getAttribute("allpatients");
		for(patients p:patient){
	%>
	
	<tr>
		<td
			 onclick="location.href='OPDm.jsp?id=<%=p.getPid()%>'">
		<%
			out.print(p.getPid());
		%>
		</td>
		...
		<td
			...
			onclick="location.href='OPDm.jsp?id=<%=p.getPid()%>'">
			<%
				out.print(p.getDiagnose());
			%>	
			</td>
			<td
			onclick="location.href='OPDm.jsp?id=<%=p.getPid()%>'">
			<%
				out.print(p.getDrugs());
			%>	
			</td>
			<td> 
				<a href="patientResult.jsp?u=<%=p.getPid()%>">Edit</a>
			    </td>	
	</tr>
